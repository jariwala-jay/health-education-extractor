...
from app.models.recipe import RecipeOriginal, RecipeCurated
...
class DatabaseCleaner:
    ...
    async def get_database_stats(self) -> dict:
        ...
        try:
            ...
            # Count recipes if they exist
            recipe_original_count = 0
            recipe_curated_count = 0
            try:
                recipe_original_count = await RecipeOriginal.count()
                recipe_curated_count = await RecipeCurated.count()
            except Exception:
                pass  # Recipe collections might not exist
            
            return {
                "health_articles": health_articles_count,
                "pdf_documents": pdf_documents_count,
                "app_articles": app_articles_count,
                "recipe_original": recipe_original_count,
                "recipe_curated": recipe_curated_count
            }
    ...
    async def clear_recipes(self, dry_run: bool = True) -> tuple[int, int]:
        ...
        Returns:
            Tuple of (original_count, curated_count) deleted/would be deleted
        """
        try:
            original_count = 0
            curated_count = 0
            ...
            try:
                original_count = await RecipeOriginal.count()
                curated_count = await RecipeCurated.count()
            except Exception:
                logger.info("Recipe collections not found, skipping...")
                return (0, 0)
            
            logger.info(f"Found {original_count} original recipes and {curated_count} curated recipes")
            
            if not dry_run:
                if original_count > 0:
                    result = await RecipeOriginal.delete_all()
                    logger.info(f"Deleted {result.deleted_count} original recipes")
                    original_count = result.deleted_count
                
                if curated_count > 0:
                    result = await RecipeCurated.delete_all()
                    logger.info(f"Deleted {result.deleted_count} curated recipes")
                    curated_count = result.deleted_count
            
            return (original_count, curated_count)
    ...
    async def clear_all(self, dry_run: bool = True, include_recipes: bool = False):
        """Clear all data from the database and file system.
        
        Args:
            dry_run: If True, only show what would be deleted
            include_recipes: If True, also clear recipe data
        """
        ...
        recipe_original_deleted = 0
        recipe_curated_deleted = 0
        if include_recipes:
            recipe_original_deleted, recipe_curated_deleted = await self.clear_recipes(dry_run)
        ...
        if include_recipes:
            logger.info(f"{action} {recipe_original_deleted} original recipes")
            logger.info(f"{action} {recipe_curated_deleted} curated recipes")
